...
def calculate_student_placements():
    ...
    second_semester_run_dates = [
        ...
        *SchoolEvent.objects.filter(event_type__in=SEMESTER_END_EVENTS, academic_year_calendar=current_calendar).values_list('ends_at', flat=True)
    ]
    ...
    for study_class in StudyClass.objects.filter(
        ...
        class_averages_ranking_sem1 = sorted(list(class_averages_ranking_sem1), reverse=True)
        class_averages_ranking_sem2 = sorted(list(class_averages_ranking_sem2), reverse=True)
        class_averages_ranking_annual = sorted(list(class_averages_ranking_annual), reverse=True)
        ...
        if school_id not in school_rankings:
            ...
            school_averages_ranking_sem1 = sorted(list(school_averages_ranking_sem1), reverse=True)
            school_averages_ranking_sem2 = sorted(list(school_averages_ranking_sem2), reverse=True)
            school_averages_ranking_annual = sorted(list(school_averages_ranking_annual), reverse=True)
        ...
        for catalog in catalogs:
            ...
            catalog.school_place_by_abs_sem1 = school_rankings[school_id]['school_absences_ranking_sem1'].index(catalog.abs_count_sem2) + 1
            ...
            if today in second_semester_run_dates:
                ...
                catalog.school_place_by_avg_sem2 = school_rankings[school_id]['school_averages_ranking_sem2'].index(catalog.abs_count_sem2) + 1
