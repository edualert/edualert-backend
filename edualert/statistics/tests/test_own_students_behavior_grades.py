...
@ddt
class OwnStudentsBehaviorGradesTestCase(CommonAPITestCase):
    @classmethod
    def setUpTestData(cls):
        ...
        cls.expected_fields = ['id', 'student', 'behavior_grade_sem1', 'behavior_grade_annual', 'behavior_grade_limit']
        cls.student_expected_fields = ['id', 'full_name']

        cls.url = reverse('statistics:own-students-behavior-grades')

    def test_own_students_behavior_grades_unauthenticated(self):
    ...
    def test_own_students_behavior_grades_wrong_user_type(self, user_role):
    ...
    def test_own_students_behavior_grades_is_not_class_master(self):
    ...
    def test_own_students_behavior_grades_first_semester(self, mocked_method):
        self.client.login(username=self.teacher.username, password='passwd')
        ...
        catalog1 = StudentCatalogPerYearFactory(student=UserProfileFactory(user_role=UserProfile.UserRoles.STUDENT, school_unit=self.school_unit, student_in_class=self.study_class),
                                                study_class=self.study_class, behavior_grade_sem1=9, behavior_grade_annual=8)
        catalog2 = StudentCatalogPerYearFactory(student=UserProfileFactory(user_role=UserProfile.UserRoles.STUDENT, school_unit=self.school_unit, student_in_class=self.study_class),
                                                study_class=self.study_class, behavior_grade_sem1=8, behavior_grade_annual=9)
    ...
    def test_own_students_behavior_grades_second_semester(self, mocked_method):
        self.client.login(username=self.teacher.username, password='passwd')
