...
@ddt
class OwnAbsencesEvolutionTestCase(CommonAPITestCase):
    @classmethod
    ...
    def test_own_absences_evolution_unauthenticated(self):
    ...
    def test_own_absences_evolution_wrong_user_type(self, user_role):
    ...
    @patch('django.utils.timezone.now', return_value=datetime.datetime(2020, 6, 16).replace(tzinfo=utc))
    def test_own_absences_evolution_month_filter(self, mocked_method):
        self.client.login(username=self.student.username, password='passwd')
        ...
        response = self.client.get(self.url, {'month': 5})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.get(self.url, {'month': 5, 'by_category': 'true'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 31)
