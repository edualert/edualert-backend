...
@ddt
class AcademicProgramsAveragesTestCase(CommonAPITestCase):
    @classmethod
    def setUpTestData(cls):
        ...
        cls.url = reverse('statistics:program-averages')
        cls.expected_fields = ['id', 'name', 'avg_sem1', 'avg_annual']

    def test_academic_programs_averages_unauthenticated(self):
    ...
    def test_academic_programs_averages_wrong_user_type(self, user_role):
    ...
    def test_academic_programs_averages_first_semester(self, mocked_method):
        self.client.login(username=self.principal.username, password='passwd')
        AcademicProgramFactory(avg_sem1=1, avg_annual=3, school_unit=self.school_unit)
        AcademicProgramFactory(avg_sem1=2, avg_annual=2, school_unit=self.school_unit)
        AcademicProgramFactory(avg_sem1=3, avg_annual=1, school_unit=self.school_unit)
    ...
    def test_academic_programs_averages_second_semester(self, mocked_method):
        self.client.login(username=self.principal.username, password='passwd')
        AcademicProgramFactory(avg_sem1=1, avg_annual=1, school_unit=self.school_unit)
        AcademicProgramFactory(avg_sem1=2, avg_annual=2, school_unit=self.school_unit)
        AcademicProgramFactory(avg_sem2=3, avg_annual=3, school_unit=self.school_unit)
