...
@ddt
class OwnActivityHistoryTestCase(CommonAPITestCase):
    @classmethod
    def setUpTestData(cls):
        cls.school_unit = RegisteredSchoolUnitFactory(academic_profile=SchoolUnitProfileFactory(name='Artistic'))
        ...
        cls.catalog = StudentCatalogPerSubjectFactory(student=cls.student, study_class=cls.study_class)
    ...
    def test_own_activity_history_unauthenticated(self):
    ...
    def test_own_activity_history_wrong_user_type(self, user_role):
    ...
    def test_own_activity_history_grades_and_absences(self):
        ...
        self.assertFalse(response.data[0]['is_coordination_subject'])
        ...
        self.assertFalse(response.data[1]['is_coordination_subject'])
        ...
        self.assertFalse(response.data[2]['is_coordination_subject'])
        ...
        self.assertTrue(response.data[3]['is_coordination_subject'])
        self.assertEqual(response.data[3]['behavior_grade_limit'], 6)

    def test_own_activity_history_exam_grades(self):
        self.client.login(username=self.student.username, password='passwd')
        yesterday = (timezone.now() - timezone.timedelta(days=1)).replace(tzinfo=utc)
        two_days_ago = (timezone.now() - timezone.timedelta(days=2)).date()
        three_days_ago = (timezone.now() - timezone.timedelta(days=3)).date()
        ...
        self.assertFalse(response.data[0]['is_coordination_subject'])
        self.assertEqual(response.data[0]['grade_limit'], 6)
        ...
        self.assertFalse(response.data[1]['is_coordination_subject'])
        self.assertEqual(response.data[1]['grade_limit'], 6)
        ...
        self.assertFalse(response.data[2]['is_coordination_subject'])
        self.assertEqual(response.data[2]['grade_limit'], 6)
