...
@ddt
class OwnStudentsAbsencesTestCase(CommonAPITestCase):
    @classmethod
    def setUpTestData(cls):
        ...
        cls.expected_fields = ['id', 'student', 'unfounded_abs_count_sem1', 'unfounded_abs_count_annual']
        cls.student_expected_fields = ['id', 'full_name']

        cls.url = reverse('statistics:own-students-absences')

    def test_own_students_absences_unauthenticated(self):
    ...
    def test_own_students_absences_wrong_user_type(self, user_role):
    ...
    def test_own_students_absences_is_not_class_master(self):
    ...
    def test_own_students_absences_first_semester(self, mocked_method):
        self.client.login(username=self.teacher.username, password='passwd')
        ...
        catalog1 = StudentCatalogPerYearFactory(student=UserProfileFactory(user_role=UserProfile.UserRoles.STUDENT, school_unit=self.school_unit, student_in_class=self.study_class),
                                                study_class=self.study_class, unfounded_abs_count_sem1=9, unfounded_abs_count_annual=10)
        catalog2 = StudentCatalogPerYearFactory(student=UserProfileFactory(user_role=UserProfile.UserRoles.STUDENT, school_unit=self.school_unit, student_in_class=self.study_class),
                                                study_class=self.study_class, unfounded_abs_count_sem1=8, unfounded_abs_count_annual=12)
    ...
    def test_own_students_absences_second_semester(self, mocked_method):
        self.client.login(username=self.teacher.username, password='passwd')
